...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        num_embeddings = len(dictionary)
        # self.project_out_dim = Linear(embed_dim, num_embeddings)
        if args.e2s2_shuffle != 0.0 and args.e2s2_random != 0.0:
            self.project_out_dim = Linear(embed_dim, 3)
        elif args.e2s2_shuffle == 0.0 and args.e2s2_random == 0.0:
            self.project_out_dim = None
        else:
            self.project_out_dim = Linear(embed_dim, 2)
    ...
    def forward_scriptable(
        ...
        for source in src_tokens:
            assert (source >= 0).all()
            assert (source[1:-1] >= 1).all()
            assert (source <= 50265).all()
            assert source[0] == 0
            #assert source[-1] == 1
        ...
        if self.project_out_dim == None:
            ...
            x_pro=x.clone()
            x_pro=self.dropout_module(x_pro)
            x_pro = self.project_out_dim(x_pro).transpose(0,1)
            return {
                "encoder_out": [x],  # T x B x C
                "encoder_out_tokens":[x_pro],
                "encoder_padding_mask": [encoder_padding_mask],  # B x T
                "encoder_embedding": [encoder_embedding],  # B x T x C
                "encoder_states": encoder_states,  # List[T x B x C]
                "src_tokens": [],
